...
class GpuSlabHash<KeyT, ValueT, SlabHashTypeT::ConcurrentMap> {
 ...
 public:
  ...
  void insertUpdate(KeyT* d_key,ValueT*d_index,uint32_t totkmers, int kmer_len);
  void buildBulk(KeyT* d_key, ValueT* d_value, uint32_t num_keys);
  void searchIndividual(KeyT* d_query,KeyT* d_index, ValueT* d_result, uint32_t totkmers, int kmer_len);
